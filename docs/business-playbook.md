...
Use Stripe Payment Links first so customers can pay without a sales call. The website reads live checkout URLs from `site/checkout.js`, and `npm run payments:check -- --live` verifies the links, success page, legal/refund page, and Starter Kit deliverables before launch. Move to Stripe Checkout plus webhooks and license keys when paid usage is proven; `examples/stripe-fulfillment-worker/` is the starter backend for that step.
