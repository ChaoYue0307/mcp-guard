...
| MCP080 | Critical | Container MCP server runs in privileged mode | Docker or Podman MCP server startup arguments include privileged container mode. | Remove privileged mode and grant only the specific capabilities, devices, and filesystem paths the MCP server needs. |
| MCP081 | Critical | Container MCP server mounts the Docker socket | Docker or Podman volume or bind-mount arguments expose `/var/run/docker.sock` or `/run/docker.sock` to the MCP server. | Do not mount the Docker socket into an MCP server. Use a narrowly scoped broker or dedicated API with least-privilege authorization. |
| MCP082 | High | Container MCP server uses host networking | Docker or Podman MCP server startup arguments use host networking. | Use a dedicated bridge network and expose only the ports required by the MCP server. |
| MCP083 | Medium/High | Container volume grants broad host filesystem access | Docker or Podman volume or bind-mount arguments expose root, home, or broad user folders to the MCP server. | Mount a narrow project directory as read-only where possible, instead of root, home, or broad user folders. |
